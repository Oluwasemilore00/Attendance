...
export default function Records() {
  ...
  const [selectedCourse, setSelectedCourse] = useState(null);   // { course_id, course_code, course_name }
  const [selectedSession, setSelectedSession] = useState(null); // { session_id, session_title, session_date }
  ...
  useEffect(() => { setSelectedCourse(null); setSelectedSession(null); }, [ownerFilter]);
  ...
  // Level 1: group all records by course
  const courseGroups = useMemo(() => {
    const map = {};
    records.forEach((r) => {
      if (!r.course_id) return;
      if (!map[r.course_id]) {
        map[r.course_id] = { course_id: r.course_id, course_code: r.course_code, course_name: r.course_name, records: [] };
      }
  ...
  // Level 2: group selected course's records by session
  const sessionGroups = useMemo(() => {
    if (!selectedCourse) return [];
    const courseRecords = records.filter((r) => r.course_id === selectedCourse.course_id);
    const map = {};
    courseRecords.forEach((r) => {
      if (!map[r.session_id]) {
        map[r.session_id] = { session_id: r.session_id, session_title: r.session_title, session_date: r.session_date, records: [] };
      }
  ...
  // Level 3: records for selected session
  const displayRecords = useMemo(() => {
    if (!selectedSession) return [];
    return records.filter((r) => r.session_id === selectedSession.session_id);
  }, [records, selectedSession]);

  // Navigation helpers
  const goToCourses = () => { setSelectedCourse(null); setSelectedSession(null); };
  const goToSessions = () => setSelectedSession(null);

  const level = !selectedCourse ? "courses" : !selectedSession ? "sessions" : "records";
      ...
      <div className="field-row" style={{ alignItems: "flex-end" }}>
        <div className="form-group" style={{ flex: 2, marginBottom: 0 }}>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && load()} placeholder="Start typing…" />
        </div>
  ...
  return (
    <div>
      ...
      <div className="page-head">
        <div>
          <h1>
            {level === "courses" && (ownerName ? `${ownerName}'s Records` : "Attendance Records")}
            {level === "sessions" && `${selectedCourse.course_code} — ${selectedCourse.course_name}`}
            {level === "records" && (selectedSession.session_title || `Session ${selectedSession.session_id}`)}
          </h1>
          <div className="breadcrumb">
            <Home size={13} /> Dashboard /
            {ownerName && <><Link to="/admin"> Administration</Link> /</>}
            {" "}
            {level === "courses" && "Records"}
            {level === "sessions" && (
              <><span style={{ cursor: "pointer", textDecoration: "underline" }} onClick={goToCourses}>Records</span> / {selectedCourse.course_code}</>
            )}
            {level === "records" && (
              <><span style={{ cursor: "pointer", textDecoration: "underline" }} onClick={goToCourses}>Records</span> / <span style={{ cursor: "pointer", textDecoration: "underline" }} onClick={goToSessions}>{selectedCourse.course_code}</span> / {selectedSession.session_title || `Session ${selectedSession.session_id}`}</>
            )}
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {level === "records" && (
            <button className="btn ghost" onClick={goToSessions}>
              <ArrowLeft size={15} /> Sessions
            </button>
          )}
          {level === "sessions" && (
            <button className="btn ghost" onClick={goToCourses}>
              <ArrowLeft size={15} /> All Courses
            </button>
          )}
          {level === "courses" && ownerName && (
          ...
          {courseGroups.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14, padding: "16px 20px" }}>
              {courseGroups.map((g) => {
                ...
                return (
                  <div key={g.course_id} onClick={() => setSelectedCourse(g)}
          ...
          {sessionGroups.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14, padding: "16px 20px" }}>
              {sessionGroups.map((g) => {
                const flagged = g.records.filter((r) => r.attendance_status === "flagged").length;
                return (
                  <div key={g.session_id} onClick={() => setSelectedSession(g)}
                    style={{ border: "1px solid var(--border)", borderRadius: 10, padding: "16px 18px", cursor: "pointer", background: "var(--bg-card,#fff)", transition: "box-shadow 0.15s, transform 0.15s" }}
                    onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 4px 16px rgba(79,70,229,0.13)"; e.currentTarget.style.transform = "translateY(-2px)"; }}
                    onMouseLeave={(e) => { e.currentTarget.style.boxShadow = ""; e.currentTarget.style.transform = ""; }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
